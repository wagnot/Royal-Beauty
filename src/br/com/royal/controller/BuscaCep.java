...
/**
 * Ferramenta de busca de CEP, veja o m�todo {@link BuscaCep#searchCep(String)} para
 * maiores informa��es.
 * <BR>
 * <BR>Constroi um objeto {@link BuscaCep} com os dados XML encapsulados, a partir
 * da chamada do m�todo estatico {@link BuscaCep#searchCep(String)}.
 * <BR>
 ...
 * <BR>
 * <BR>Exemplo de uso:
 * <BR><tt>{@link BuscaCep} cep = {@link BuscaCep}.searchCep("13345-325");
 * 
...
/* Classes Internas, que auxiliam na busca do CEP */
	 ...
	 * {@link Xml#setCep(String, BuscaCep)}.
	 * @author Tomaz Lavieri
	 */
	private enum Xml {
		CIDADE {
			@Override public void setCep(String text, BuscaCep webServiceCep) {
		 ...
		 * {@link BuscaCep} no seu parametro correspondente. Cada constante do enum
		 * conhece o seu parametro a passar.
		 * @param text {@link String} contendo o texto a ser inserido.
		 * @param webServiceCep {@link BuscaCep} referencia do objeto para inserir
		 * 		  o parametro text.
		 */
		public abstract void setCep(String text,BuscaCep webServiceCep);
	}
	/**
...
/* M�todos e variaveis estaticas, respons�veis pela busca do CEP */
	 ...
	 * @return {@link BuscaCep} contendo as informa��es da pesquisa.
	 */
	public static BuscaCep searchCep(String cep) {
		cep = cep.replaceAll( "\\D*", "" ); //To numeric digits only
		if (cep.length() > 8)
			cep = cep.substring(0, 8);
		BuscaCep loadCep = new BuscaCep(cep);
		try {
