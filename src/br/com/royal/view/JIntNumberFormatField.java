 ...
 */
public class JIntNumberFormatField extends JTextField {
	private static final long serialVersionUID = -7506506392528621022L;
	private NumberFormat numberFormat;
	private int limit = -1;

//	public JNumberFormatField(int casasDecimais) {
//		this(new DecimalFormat((casasDecimais == 0 ? "#,##0" : "#,##0.")
//				+ makeZeros(casasDecimais)));
//	}

	public JIntNumberFormatField() {

		this(NumberFormat.getIntegerInstance());
	}

	public JIntNumberFormatField(NumberFormat format) {// define o formato do
		// número
		...
		setDocument(new PlainDocument() {
			...
			public void insertString(int offs, String str, AttributeSet a)
				...
				String text = new StringBuilder(JIntNumberFormatField.this
						.getText().replaceAll("[^0-9]", "")).append(
				...
				super.insertString(0, numberFormat.format(
						new BigDecimal(
						getLimit() > 0 == text.length() > getLimit() ? text
								.substring(0, getLimit()) : text)
						.divide(new BigDecimal(Math.pow(10,
								numberFormat.getMaximumFractionDigits())))), a);
			}
	...
	private static final String makeZeros(int zeros) {
		if (zeros >= 0) {
			...
			throw new RuntimeException("Número de casas decimais inválida ("
					+ zeros + ")");
		}
	}
